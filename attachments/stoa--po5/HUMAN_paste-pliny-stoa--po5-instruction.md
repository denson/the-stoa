...
## What this is
You orchestrate **arc-77** — the **u--9s2 inc 2.4 Phase-1 BUILD** of the secure-core credentialed per-provider pass-through service. You dispatch CHIRON+HAMILTON (brief-revision) then the full gauntlet (DAEDALUS → ARGUS → ADA → VERA → CATO → NOMOS), sequentially, verifying at each breakpoint.
...
Then read the design being built: `agents/design/stoa--q7f/design-rev2.md` (arc-76/build worktree, commit 26698475) + the runbook `docs/secure-core/deployment-plan.md`. DAEDALUS reads design-rev2 in full.
...
### Sub-phase A — CHIRON+HAMILTON build-brief revision (FIRST)
Before the build gauntlet, CHIRON+HAMILTON revise the build brief to:
1. **Reframe the core as the CONSOLIDATION center** all lanes consume (science sos--373 + newswire store nws-1n7 + future builders) — multi-consumer identity/authz on the pass-through is first-class, not a science detail.
2. **Fold the mandatory canons (Grand-ratified):** seal-every-secret + fail-closed seal-audit (u--84m); Railway-setup skill (sos--1bk) as the Phase-2 provisioning reference; the **in-harness-workflows canon** (`docs/research/anthropic-workflows-report.md`) — LLM-adjudication runs in-harness on subscription OAuth, never a keyed CI call, never via CLAUDE_CODE_OAUTH_TOKEN for a shared pipeline. **HAMILTON records the workflow-vs-app-code call honestly** — do not manufacture a Stoa Workflow that isn't load-bearing (the prior inc ratified engine=app-code/no-workflow for SUGGEST; the same honest judgment applies).
