...
- **Arc 20 — User-tier install convention + dir choice + `{{USER_TIER_DIR}}` substitution.** `install.sh --target user` now detects existing `user-beadwork/` at common locations (`~/stoa_projects/`, `~/claude_projects/`, `~/projects/`, `~/Code/` — by `beadwork` orphan branch presence), surfaces a confirmation prompt, and if none found prompts for the user-tier directory choice (default `~/stoa_projects/`). Strict scaffolding follows: `mkdir + git init + bw init`. The chosen path substitutes `{{USER_TIER_DIR}}` in the deployed `~/.claude/MAJOR_POLYBIUS.md`. Closes the install-side gap caught when a fresh user-tier POLYBIUS spawned in home directory failed to find user-beadwork. Substrate edits at `substrate/install.sh` + `substrate/MAJOR_POLYBIUS.md` §7.5; install-stoa skill Beat 3a walks the dialog.
