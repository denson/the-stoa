# Arc 28 design REV4 — bw 0.13.0 substrate adoption (B.3 + B.4) + bw-upgrade discipline (C.1 + C.2)
...
**Supersedes:** `design.md` (rev1) per the ARGUS rev1 verdict at `stoa--s6n` 2026-05-17T03:07:43Z; `design-rev2.md` per the ARGUS rev2 verdict at `stoa--s6n` 2026-05-17T03:12:56Z; and `design-rev3.md` per the ARGUS rev3 verdict at `stoa--s6n` 2026-05-17T03:18:30Z. Rev1, rev2, and rev3 all stay on disk as history per substrate convention.

---

## Rev4 changelog — what was revised + why

The ARGUS cold-audit of rev3 (verdict timestamped 2026-05-17T03:18:30Z) returned REVISE narrow with **rev2's NEW-1 RESOLVED at Probe 7b** (PATH-prefixed curl-shim substitution traced end-to-end on Git-Bash; defang catches; guard fires; unreachable message emits; exit 0) and exactly **one NEW load-bearing risk + one advisory** introduced at the smoke-beat rendering surface. Rev4 is rev3 with surgical substitution to one affected surface (and one documentation-drift correction); every other section is verbatim from rev3.

**NEW-2 (rev3-introduced; rev4 closes):** §5 Smoke beat 4 (rev3 lines 887-902) wrapped the curl-shim setup/exercise/teardown inside a markdown numbered-list item with 3-space indentation. The closing `   SHIM` heredoc terminator at rev3 line 892 has leading whitespace; plain `<<'SHIM'` (no dash) requires the terminator at column 1. `bash -n` empirically emits `warning: here-document at line N delimited by end-of-file (wanted 'SHIM')` on the as-pasted text. The cascade: chmod / exercise / teardown lines all get swallowed as heredoc content; the shim never gets the executable bit; the PATH-prefixed run fails for the wrong reason; the smoke beat's PASS condition is unsatisfiable. Probe 7b at rev3 lines 802-818 was already at column 0 (unindented fenced block at the outer markdown structure) and is empirically clean — same shape, different presentation.

**Picked: de-indent the smoke beat 4 code block to column 0 (option α in PLINY's rev4 brief).** Justification:

1. **Mirrors Probe 7b's working form exactly.** Probe 7b's column-0 fenced block was traced empirically clean by ARGUS rev3. Adopting the same presentation for Smoke beat 4 collapses the two surfaces to the same column-0 shape — eliminating the markdown-indentation footgun entirely rather than working around it.
2. **Lowest novelty.** Option β (`<<-'SHIM'` with TAB-indented terminator) is mechanically correct per bash docs but fragile in editing — only TABs are stripped, never spaces; an editor that auto-expands TABs silently breaks the heredoc. Option γ (replace heredoc with `printf '#!/bin/sh\nexit 7\n' > /tmp/curlshim/curl`) loses the readability advantage of an inline shim body. Option α adds zero new mechanisms.
3. **Unambiguous markdown semantics.** A column-0 fenced code block is a top-level markdown structure, not a list-item child. Renderers treat it as a sibling of the numbered list, not nested inside it. Some loss of visual nesting between the smoke beat's narrative bullet ("4. **LBR-1 defang smoke…**") and the code block, but that loss is cosmetic; the structural correctness of the rendered code is load-bearing.

**Rejected alternatives (for the record):**

- **Option β — `<<-'SHIM'` with TAB-indented terminator.** Bash docs: `<<-` strips leading TABs (only TABs, never spaces) from the heredoc body AND the terminator. Mechanically correct, empirically fragile: any editor or git filter that converts TABs to spaces silently breaks the heredoc with no compile-time signal. Rejected on robustness-of-presentation grounds.
- **Option γ — `printf '#!/bin/sh\nexit 7\n' > /tmp/curlshim/curl` (no heredoc).** Mechanically clean and lint-trivial, but the inline-shim-body readability suffers; a reader scanning the smoke beat has to mentally parse the `\n`-separated body. Probe 7b chose heredoc for the same readability reason; keeping the two surfaces shape-identical (per Arc 27 smoke-beat redundancy discipline) is more valuable than the marginal lint simplification. Rejected on read-clarity grounds.

**ADV-1 (rev3-introduced; rev4 corrects accurately):** Rev3's §7 added one new bullet ("Adding a script-level URL override env-var…") at rev3 line 983; rev3's changelog (rev3 line 37) listed §7 as "verbatim from rev2 (no changes)." Documentation drift, substance sound. **Picked: Option Y** — Rev4 carries §7 forward verbatim from rev3 (i.e., includes the bullet rev3 added), and this Rev4 changelog accurately attributes the addition to rev3 (the historical truth) rather than retroactively rewriting rev3's changelog. This preserves rev-by-rev attribution honesty: rev3 made the §7 change as a free fold-in of the script-level-override rejection; rev4 inherits it with correct attribution.

**MANDATORY pre-verification (per PLINY rev4 brief + §6.4 meta-discipline closure):** Before sealing rev4, DAEDALUS extracted the smoke beat 4 code block from rev4's rendered source, piped to `bash -n`, and confirmed ZERO warnings — closing the §6.4 meta-pattern instance at the smoke-beat-rendered-form layer. The discipline DAEDALUS rev3 named ("empirical pre-verification of test mechanisms before sealing them into probe/smoke text") was not applied to the smoke-beat rendering itself in rev3; rev4 applies it to its own output. The pre-verification command + output is documented in the §6.4 rev4 addendum.

**Surfaces revised in rev4 (and ONLY these):**

- §5 Smoke beat 4 — code block de-indented to column 0; narrative bullet ("4. **LBR-1 defang smoke...:**") retained at list-item indent; one-sentence parenthetical added pointing the reader at the column-0 placement and why.
- §6.4 — one added sentence at the end noting that rev4 closes the meta-pattern instance at the smoke-beat-rendered-form layer (third occurrence of the §6.4 class: rev1 LBR-1 at runtime; rev2 NEW-1 at verification-mechanism; rev3 NEW-2 at smoke-beat-rendered-form). Empirical pre-verification step is documented inline.
- This Rev4 changelog block (added; describes the rev4 change + accurately attributes the rev3 §7 addition).

**Verbatim from rev3 (no changes):** all of §1, §2.1, §2.2, §2.3, §3 (all five files), §4 (Probes 5, 6, 7, 7b, 8, 9, 10, and the authorship discipline probe), §5 (beats 1, 2, 3, 5, 6, 7, 8), §6.1, §6.2, §6.3, §7 (including the rev3-added bullet about not adding a script-level URL override env-var; that bullet stands per Option Y). The rev3 and rev2 changelog blocks below are preserved for full-history reading without consulting earlier revisions separately.

---

## Rev3 changelog — what was revised + why

The ARGUS cold-audit of rev2 (verdict timestamped 2026-05-17T03:12:56Z) returned REVISE narrow with **all rev1 risks (LBR-1/2/3 + ADV-1..5) resolved** and exactly **one NEW load-bearing risk** introduced at the verification surface. Rev3 is rev2 with surgical substitution to the two affected surfaces; every other section is verbatim from rev2.

**NEW-1 (rev2-introduced; rev3 closes):** §4 Probe 7b (rev2 lines 756-772) and §5 Smoke beat 4 (rev2 lines 832-838) both simulated upstream failure with `PATH=/nonexistent`. That breaks shebang resolution — `/usr/bin/env` cannot find `bash` on an empty PATH, so the kernel/exec layer returns exit code 127 before any line of the script runs. The probe's stated PASS condition (exit_code=0; output contains "could not reach...") is unsatisfiable by construction. VERA running either surface as written would always observe exit_code=127 and (per the probe's own PASS-condition text) conclude "the LBR-1 defang regressed." Half (b) of the LBR-1 mitigation — the durable verification surface — was destroyed by the simulation mechanism it used. ARGUS empirically verified three alternative simulation shapes; DAEDALUS rev3 picks one and applies it to both surfaces.

**Picked: PATH-prefixed curl-shim (option ii in ARGUS's rev2 verdict).** Justification:

1. **Zero production-script changes.** The script's API stays exactly as rev2 left it. No new env-var override, no new flag, no new test-only branch. The substitution is purely external to the script.
2. **Deterministic across distros.** Bash launches normally (the shim is a PATH **prefix**, not a replacement — `/usr/bin` and `/bin` remain on PATH for shebang resolution and every other tool the script invokes). The shim is what `curl` resolves to; everything else (python3, sed, tr, printf, cat, etc.) keeps working.
3. **Exercises the realistic failure mode.** The shim exits with code 7 (curl's documented DNS-failure code, per `man curl` CURLE_COULDNT_RESOLVE_HOST). That is the most realistic upstream-failure scenario the LBR-1 defang is designed to catch — closer to "the network is broken in the field" than to "the curl binary disappeared from the system."
4. **Same shim works for both surfaces.** Probe 7b and Smoke beat 4 use the identical setup, so the duplicate-shape redundancy (Arc 27 smoke-beat discipline; the two surfaces are intentionally redundant) is preserved without divergence.
5. **Cleanup is trivial.** `mkdir -p /tmp/curlshim` + `chmod +x` + `rm -rf /tmp/curlshim` at teardown. No state to coordinate across runs.

**Rejected alternatives (for the record):**

- **Bogus DNS host via inline URL override.** Would require adding a new env-var override (e.g., `BW_RELEASE_CHECK_API_URL_OVERRIDE`) to the script purely for the test surface. That widens the script's production API for a test-only need, and a "narrow rev3" should not be growing the script. Rejected on scope discipline.
- **`env -i PATH=/usr/bin:/bin` with curl pruned.** Assumes curl is NOT in `/usr/bin` or `/bin`. On Linux distributions curl is overwhelmingly installed at `/usr/bin/curl`; on Git-Bash for Windows curl also lives at `/usr/bin/curl`. The mechanism fails to simulate failure on the install in question (and almost every install). Rejected on empirical incorrectness.

**Surfaces revised in rev3 (and ONLY these):**

- §4 Probe 7b — `PATH=/nonexistent` simulation replaced with PATH-prefixed curl-shim setup + teardown.
- §5 Smoke beat 4 — same substitution (duplicate of Probe 7b's shape per smoke/probe redundancy discipline).
- §6.4 — one added sentence at the end noting that rev3 closes the meta-pattern (clone-shape defect inheritance) at the verification layer too, not just at the runtime layer. The weak-point text itself stands; the class observation is still valid; rev3 just records the additional closure.

**Verbatim from rev2 (no changes):** all of §1, §2.1, §2.2, §2.3, §3 (all five files), §4 (Probes 5, 6, 7, 8, 9, 10, and the authorship discipline probe), §5 (beats 1, 2, 3, 5, 6, 7, 8), §6.1, §6.2, §6.3, §7. The rev2 changelog block below is preserved for full-history reading without consulting rev2 separately.

---

## Rev2 changelog — what was revised + why (preserved verbatim from rev2)
...
**Probe 7b (added rev2 per LBR-1; mechanism revised rev3 per ARGUS rev2 NEW-1) — LBR-1 defang behavior under simulated upstream failure:**
```bash
# Simulate a curl failure via a PATH-prefixed shim. Bash itself launches
# normally (/usr/bin still on PATH for shebang resolution and every other
# tool the script invokes); only `curl` resolves to the shim, which exits 7
# (curl's documented CURLE_COULDNT_RESOLVE_HOST). The defang at the
# pipeline boundary in fetch_latest_release_tag catches the curl non-zero
# exit; the function echoes empty; the caller's `[ -z "${latest}" ]` guard
# fires and emit_unreachable_message runs.
#
# Picked over `PATH=/nonexistent` (rev2; kills shebang resolution before
# any line of the script runs — see Rev3 changelog) and over a bogus-host
# inline URL (would require adding a test-only env-var override to the
# script's production surface). Picked over `env -i PATH=/usr/bin:/bin`
# (would NOT prune curl on installs where curl lives at /usr/bin/curl,
# which is overwhelmingly the case).

# --- setup ---
mkdir -p /tmp/curlshim
cat > /tmp/curlshim/curl <<'SHIM'
#!/bin/sh
# CURLE_COULDNT_RESOLVE_HOST — most realistic upstream-failure mode.
exit 7
SHIM
chmod +x /tmp/curlshim/curl

# --- exercise ---
PATH=/tmp/curlshim:$PATH \
  /c/Users/denso/claude_projects/the-stoa-arc-28/substrate/skills/check-bw-release/check.sh
probe_exit_code=$?
...
rm -rf /tmp/curlshim
```
PASS condition: script exit code is 0; output contains the exact substring `could not reach https://api.github.com/repos/jallum/beadwork/releases/latest`. If exit code is non-zero, OR the "could not reach" message is absent, the LBR-1 defang regressed — fail the probe and surface to PLINY. (Note: a non-zero exit specifically of 127 means shebang resolution failed — that is a probe-mechanism regression, not a script regression; check `/tmp/curlshim/curl` was actually created with the executable bit before re-running.)
...
Per directive Phase C + project-tier POLYBIUS's 03:00:04Z trim + rev2 additions (one beat added for LBR-1 defang verification; one beat updated for the per-workspace state-file path) + rev3 substitution of the LBR-1 defang beat's failure-simulation mechanism (per ARGUS rev2 NEW-1). PLINY runs these before opening the PR:
...
4. **LBR-1 defang smoke (added rev2; mechanism revised rev3 per ARGUS rev2 NEW-1; code block de-indented to column 0 rev4 per ARGUS rev3 NEW-2 — see Rev4 changelog for the indented-heredoc-terminator footgun this avoids):**

(Code block deliberately at column 0 — top-level fenced block, not nested inside the numbered-list item — so the `SHIM` heredoc terminator sits at column 1 of the bash source as the `<<'SHIM'` form requires. Empirically pre-verified clean via `bash -n` before sealing rev4; see §6.4 rev4 addendum.)

```bash
# Simulate curl failure via PATH-prefixed shim (bash launches normally;
# only `curl` resolves to the shim, which exits 7 = CURLE_COULDNT_RESOLVE_HOST).
# Duplicate of §4 Probe 7b's shape on purpose (Arc 27 smoke-beat
...
PATH=/tmp/curlshim:$PATH \
  /c/Users/denso/claude_projects/the-stoa-arc-28/substrate/skills/check-bw-release/check.sh
smoke_exit_code=$?
echo "exit_code=${smoke_exit_code}"

# --- teardown ---
rm -rf /tmp/curlshim
```

   Expected: script emits "could not reach https://api.github.com/repos/jallum/beadwork/releases/latest" and exits 0. If exit code is non-zero, the LBR-1 defang regressed — stop and surface to PLINY before opening the PR. (Exit code 127 specifically indicates probe-mechanism regression — shim setup failure — not a script regression; verify the shim file was created with the executable bit before re-running.)
...
## §6 — Self-assessed weak points (for ARGUS, rev3 lens)

Three places where ARGUS should look hardest at the rev2/rev3 picks. Honest, scoped to what the picks rest on. Rev1's §6.1 and §6.2 are resolved into the design and removed; rev1's §6.3 (N=1 framing) is preserved as ratified; rev1's §6.4 (clone-shape defect inheritance) is preserved with a rev2 lens — the specific LBR-1 instance is fixed at the runtime layer, the rev2 NEW-1 instance (the verification surface that mistakenly used `PATH=/nonexistent`) is fixed at rev3 with a PATH-prefixed curl-shim, and the broader risk class persists.
...
**Rev3 addendum:** the meta-pattern named above (clone-shape defect inheritance) manifested AGAIN at the verification layer in rev2 — the `PATH=/nonexistent` failure-simulation in Probe 7b and Smoke beat 4 was structurally another instance of the same "looks right at a glance; subtly broken under the actual runtime preconditions" defect class, this time at the test-mechanism surface rather than the script surface. Rev3 closes that specific instance (PATH-prefixed curl-shim) but does NOT structurally prevent the class — the same observation applies to any future test-mechanism the skill or its siblings introduce. The mitigation is the same shape: empirical pre-verification of test mechanisms before sealing them into probe/smoke text (ARGUS empirically verified the three rev3 alternatives before naming them; DAEDALUS verified the picked shape mechanically by tracing it through the script's existing code path before drafting). If a future change adds a new probe or smoke beat that exercises a failure path, ADA should run it once against the actual skeleton before sealing it in.

**Rev4 addendum:** the meta-pattern manifested a THIRD time, one rendering layer down — rev3 verified the curl-shim mechanism but did not verify the rendered markdown form of Smoke beat 4 carrying it. The 3-space markdown list-item indentation pushed the heredoc terminator off column 1, and `bash -n` against the as-rendered text emitted `warning: here-document at line N delimited by end-of-file (wanted 'SHIM')` — cascading every subsequent line into the heredoc body and silently breaking chmod/exercise/teardown. Rev4 closes that instance (column-0 fenced code block) AND applies the §6.4 discipline DAEDALUS rev3 named-but-did-not-apply to its own output: before sealing rev4, DAEDALUS extracted the smoke beat 4 code block to a scratch file (`/tmp/rev4-smoke4-test.sh`) and ran `bash -n` against it — exit 0, no warnings, "SYNTAX CLEAN" emitted by the verification wrapper. The class is still not structurally prevented — any future probe/smoke beat that embeds a heredoc inside a markdown list item recreates the footgun — but the rev4 closure pattern is now: write probe/smoke text → extract the rendered code block → `bash -n` it → only then seal. ADA carries the same discipline forward: any new probe/smoke beat that exercises shell mechanisms gets `bash -n` against the as-extracted form before commit, not just against the live script the prose surrounds.
...
- Adding a script-level URL override env-var (`BW_RELEASE_CHECK_API_URL_OVERRIDE` or equivalent) for the test surface. The rev3 PATH-prefixed curl-shim works without it; growing the script's production API for a test-only need is the wrong direction.
