...
- **stoa--3na** (P3) — apply substrate canon (Arcs 38-41 + Pass 8) to ariadne + sector-4 + railway workspaces. Filed Pass 8 post-Arc-41 drift-sweep verification per §13.13 criterion 4. PRINCIPAL ratified option (a) "apply to the-stoa only now; defer others" at 2026-05-18; the-stoa project-tier .claude/ deployed (commit 51e0b9f); ariadne/sector-4/railway require separate per-workspace apply sessions (interactive consent + role-file-modified-while-agent-running warnings). NOT a blocker for spec-met (Pass 10 stellation fires fresh install.sh at new workspace; cross-substrate workspaces don't affect validation). Arc 43+ OR routine post-spec-met substrate-deploy maintenance.
- **stoa--4zj** (P3) — refine validate-spec parsers (check-1 §-ref resolver heading-skip heuristic + check-2 claim-status disambiguation). Filed mid-Arc-42 per Pass 9 first-run WP-1 residue (144/274 §-refs FAIL + 36 STRANGE classifications). Parser limitation anticipated at Arc 42 design time (DAEDALUS self-flagged; ARGUS accepted as future-arc work); A20 motivated-reasoning property HELD (validate-spec produced honest FAILs vs. auto-PASS); the residue IS the operationally-acceptable fix-point per stoa--bbi refined-principle thesis. Arc 43+ scope; not a blocker for spec-met (Pass 9 produces artifact with honest PASS/FAIL classification + residue documented = §13.13 criterion 5 satisfied; not all-green).
...
- `check-substrate-updates` skill returns "no drift" across registered consumer workspaces (including user-tier per Arc 38 C2 / stoa--bj5). **LOOSE interpretation per Arc 42 A21 PRINCIPAL ratification (2026-05-18)**: drift across (a) ariadne-core-workspace / sector-4 / railway_stoa (per stoa--3na deferral) AND (b) the-stoa workspace itself (substrate-authoring tier; substrate evolves faster than .claude/ install can keep up during active arc-build per MAJOR_POLYBIUS.md §18 self-apply exception) is **documented-residue, not BLOCK**. The check satisfies-with-residue when these 4 workspaces emit drift AND mech-check-results.md documents the residue with reference to this paragraph. Future-arc tightening to strict requires either stoa--3na closure + per-arc-build install.sh-rerun discipline OR substrate-authoring-tier-self-drift structurally addressed via separate canon (e.g., post-merge install.sh autorun).
