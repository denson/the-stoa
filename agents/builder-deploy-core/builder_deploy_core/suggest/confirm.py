...
# prototype-source: agents/design/stoa--jw5/suggest-check/confirm.py (35/35 PASS)
# design-ground-truth: agents/design/stoa--fdf/design-rev2.md §2.4 (fail-closed gate, logic-verbatim) /
#                      §2.5 + §4-decision #3 (the evidence-rich anti-rubber-stamp contract layer) ;
#                      design-formal §26 (human-CONFIRM FAIL-CLOSED gate) / §26.1 (D := confirm(P)) /
#                      §26.2 (HITL IS the safety mechanism)
#
...
#
# PROMOTION: the _Inert sentinel + confirm() + is_declare() are LOGIC-VERBATIM from the prototype
# (no algorithm edit). What 2.2 ADDS is the contract-aware confirm_presented() entrypoint (§2.5 /
# §4-#3 invariant 1: NO bulk blind-accept — a CONFIRM is honored only against a MATERIALIZED
# CandidatePresentation), layered on top of (not replacing) the verbatim confirm().
#
# Provisions NOTHING; reads NO environment. Pure function of (proposed, human_action).

from __future__ import annotations

from builder_deploy_core.suggest.evidence import check_contract
...
def confirm_presented(presentation, proposed, human_action):
    """§2.5 / §4-#3 invariant 1 (NO bulk blind-accept) — the CONTRACT-AWARE confirm entrypoint.

    A CONFIRM is honored ONLY against a MATERIALIZED CandidatePresentation that satisfies the
    anti-rubber-stamp invariants (every proposed candidate presented WITH non-empty evidence + a source
    tag). If the presentation is missing/incomplete (a confirm against an un-presented proposal), the
    gate FAILS CLOSED (-> INERT) regardless of the human action — there is no path to DECLARE that
    bypasses the evidence presentation.

    Inputs:
      presentation: the list[CandidatePresentation] materialized for `proposed` (evidence.build_presentation).
      proposed:     the suggest() proposed service-id list.
      human_action: the §26 human action (see confirm()).

    Returns a DECLARE list (on a valid presentation + CONFIRM) or INERT (fail-closed) — the same
    contract as confirm(), with the added pre-gate that the evidence presentation must exist + be valid.
    The EDIT (per-candidate add/remove) and no-response (⊥) invariants ride confirm() unchanged.
    """
    contract = check_contract(presentation, proposed)
    if not contract.ok:
        # invariant 1: cannot CONFIRM against an un-presented / evidence-less proposal -> fail closed.
        return INERT
    return confirm(proposed, human_action)
