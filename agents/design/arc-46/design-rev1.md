...
| **PostCompact can/can't inject context** | .5: "reportedly cannot inject context" | CONFIRMED (corrected per ARGUS r3). **`PostCompact` DOES exist** — it fires *after* context compaction completes — but it is a **side-effect-only / "no decision control" event: it cannot inject context.** (An earlier draft of this row wrongly said "there is no PostCompact event"; ARGUS r3 corrected the fact. The earlier *operational conclusion* was right for the right reason: PostCompact's inability to inject context means there is no instant auto-reprime the moment compaction finishes.) `PreCompact` likewise fires *before* compaction and cannot carry post-compaction state forward. So the reprime-after-compaction job has no inject-capable compaction event; the correct homes are **`SessionStart` (matcher `"compact"`, which CAN inject additionalContext on a compact-triggered resume)** and **cron** (the standing re-injection). | **CONFIRMED — design routes to SessionStart/cron (§5.5, §9)** |
...
4. **PostCompact exists but cannot inject context (§2, corrected per ARGUS r3) — there is no instant post-compaction reprime.** PostCompact fires after compaction completes but is a side-effect-only / "no decision control" event: it cannot inject context. (An earlier draft wrongly said "PostCompact does not exist"; the fact is corrected, the operational conclusion is unchanged — see §2 row 6.) PreCompact fires *before* compaction and cannot carry post-compaction state forward; the only inject-capable home on a compact-triggered resume is SessionStart(matcher:compact). So the instant an auto-compaction finishes mid-turn, nothing re-primes until the NEXT trigger (next tool call's PreToolUse, next Stop, next SessionStart-on-resume, next cron). *Why this shape anyway:* this is acceptable because the deterministic PreToolUse gates fire on the very next gated tool call regardless of compaction state, and cron re-injects on cadence — the design never depended on instant reprime. But it is a real reduction vs the .5 mental model and ARGUS should weigh whether SessionStart(matcher:compact) should carry an explicit reprime payload in Stage 1 (the design currently leaves SessionStart unused).
...
- **R2 (weak point 4):** should Stage 1 add a `SessionStart` (matcher `"compact"`) reprime hook to partially recover the no-instant-reprime gap (PostCompact exists but cannot inject context — §2 r3-corrected), or is "next trigger carries it" sufficient (the design's current position)?
