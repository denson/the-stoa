...
supersedes/builds-on: design-rev4.md (arc-77/build — SHAPE cleared by ARGUS cold-audit + ARGUS rev4 re-audit + VERA build-falsification PASS; carried UNCHANGED except the ONE INV-RESP depth fold in the rev5 changelog)
consumes: docs/research/anthropic-workflows-report.md (in-harness canon), docs/secure-core/deployment-plan.md (runbook), arc-77-build-directive.md §3A/§4/§5/§6, agents/verdicts/stoa--q7f/argus-rev3-security.md (the ARGUS REVISE verdict rev4 folded), agents/verdicts/stoa--q7f/vera-build-falsification.md (the VERA PASS + the INV-RESP depth-2 finding this rev folds)
status: design-rev5 (BUILD-facing — the REAL service). Folds the ONE VERA-surfaced, FM-ratified FIX-NOW finding: INV-RESP made DEPTH-COMPLETE (recursive allow-list to schema depth; `content` the one documented whole-passthrough exception; P-M2 extended with a nested-declared-field clause; M2 honest-claim updated). NO structural redesign — everything else in rev4 carried VERBATIM (multi-lane authz, INV-PRINCIPAL, seal-audit, all other invariants/probes NOT re-litigated).
...
# Secure Railway core for the CONSOLIDATION center — credentialed per-provider pass-through, multi-lane (rev5, BUILD-facing)
...
**RECURSIVE** allow-list property (rev5): undeclared keys are dropped at EVERY depth — not just the top two
levels — so a credential nested inside a declared field is dropped by construction; `content` is the ONE
documented whole-passthrough exception (the model product, which is not credential-bearing because the ADC
bearer is a request-side header never present in a response body). **M4 closed by construction at the per-lane granularity** (per-(principal, provider,
...
- INV-RESP (per-op response/chunk allow-list serializer, RECURSIVE to schema depth; `content` the one documented whole-passthrough) — pure code.
...
| `INV-RESP` | No credential byte reaches a caller via the response — at ANY depth | per-op `response_schema`/`chunk_schema` allow-list applied **RECURSIVELY to schema depth** (an undeclared key is dropped at EVERY level); `content` the one documented `PASS_WHOLE` exception; op with no schema **OR** a legacy depth-2 (bare-`list`) node **refuses to load** | CARRIED (rev2 §1.3.1), **DEEPENED rev5 (§5.1a)** |
...
2. **`SCALAR` leaf** — the value passes **iff it contains NO object (dict) at any depth**: a JSON primitive, or
   an array of (recursively) primitives. A value carrying an object anywhere under a `SCALAR` node is
   structurally unexpected → DROP. (This is what forces every object-bearing field to be an explicit OBJECT
   node — a credential cannot ride an undeclared sub-key of an object smuggled under a `SCALAR` slot, because
...
3. **`PASS_WHOLE` leaf — the ONE explicit, documented whole-passthrough exception** (see below). The value
   ...
   is the ONLY escape from recursion.
...
**Threat-anchored probe P-M2 (EXTENDED — rev5; §6.13).** The probe exercises the M2 attack path (a credential
echoed into the response), now including the NESTED-declared-field depth. It asserts BOTH halves:
- **(a) attack-blocked:**
  ...
  - **(a.2) NESTED-DECLARED-FIELD (NEW — rev5; falsifies a depth-2 build):** plant a credential VALUE at
...
- **(b) legit-unaffected:** a DECLARED nested sub-key passes INTACT — `candidates[].groundingMetadata.webSearchQueries`
  is returned unchanged (recursion did not break the grounding feature); AND `candidates[].content` passes
  **WHOLE** (the documented `PASS_WHOLE` exception is exercised — the model product is not truncated). (The depth
  tightening did not defeat M2 by breaking a legitimate declared field or the model output.)
...
| **M2** key-exfil | INV-RESP (**RECURSIVE** allow-list to schema depth) + channel-by-construction → caller tries to read a key from a response/error/log, **including via a credential nested at depth ≥ 3 inside a declared field** → server-side cred, no caller-URL, per-op allow-list drops every UNDECLARED key at EVERY depth (recursive), `content` the one documented `PASS_WHOLE` exception (model product, not credential-bearing), opaque **or legacy depth-2 (bare-`list`)** schema refuses to load | **P-M2 EXTENDED (rev5 — §5.1a):** (a.1) undeclared field/subfield/header/error dropped [carried]; **(a.2 NEW)** credential at depth ≥ 3 in an undeclared sub-key of a declared field → DROPPED from caller + BOTH audit phases (FALSIFIES a depth-2 build); (b) declared nested sub-key intact + `content` passes whole |
...
1. Build the REAL service: `tailscaled → tailscale serve --https=443 --accept-app-caps=<per-lane caps>
   ...
   recurses to schema depth over the OBJECT/`SCALAR`/`PASS_WHOLE` node grammar; `content` the one `PASS_WHOLE`
   exception); the two-phase audit; the seal-audit gate. Vertex registered as the first provider
   (`generate_grounded` + `embed`, both `auth: adc_sa`).
2. Run **P-M1..P-M7** against the REAL code (locally/against mocks) — all PASS, INCLUDING the NEW P-M2(a.2)
   nested-declared-field clause (a credential at depth ≥ 3 in an undeclared sub-key of a declared field →
   DROPPED from caller + BOTH audit phases; a depth-2 build FAILS it), the P-M4(a.2) unresolvable-principal
   clause, and the P-M7(a.2) marked-bypass clause.
3. INV-DEST/RESP/BIND/LANE each **fail-loud verified** (plant a violation → refuse-to-serve/load; not
   ...
   passes whole). **INV-PRINCIPAL request-time fail-closed verified** (unmapped cap / absent header →
...
3. **[rev5 DEEPENED — the depth-2 gap is CLOSED] INV-RESP allow-list completeness against an EVOLVING Vertex
   response surface.** rev5 made `redact_response` **RECURSIVE to schema depth** (the VERA depth-2 finding), so
   a credential in an UNDECLARED sub-key at ANY depth is now dropped by construction, `content` is the one
   documented `PASS_WHOLE` exception, and the loader REFUSES a legacy depth-2 (bare-`list`) schema. **Two HONEST
   ...
   on:** does **P-M2(a.2)** actually falsify a depth-2 build (assert the planted depth-3 undeclared sub-key is
   EMITTED by a whole-pass build, DROPPED by the recursive build)? is the `content` `PASS_WHOLE` exception
   genuinely bounded to `content` (is `groundingMetadata` NOT whole-passed)? does the loader guard refuse the
   legacy bare-`list` node (regression fails closed at LOAD)? *Why this shape anyway:* a recursive KEY allow-list
   with explicit OPT-IN whole-pass is the by-construction form that holds for future ops/providers;
   value-content scanning belongs to the seal-audit layer, not the response serializer; fail-closed-by-omission
   is the right posture for an upstream the core does not own.
