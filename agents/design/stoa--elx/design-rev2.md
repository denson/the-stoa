...
<!-- from: CAPTAIN_DAEDALUS_the-stoa (ARCHITECT) — Phase A design deliverable, rev2 -->
<!-- supersedes: agents/design/stoa--elx/design-rev1.md (rev2 wins on any conflict) -->
<!-- consumes: substrate/arcs/arc-75-build-directive.md + beadwork:attachments/stoa--elx/plan-bw-bootstrap-v2.md (Grand-GATED) + ARGUS rev1 cold-audit NO-GO (F1-F9) -->

# Design rev2 — bw bootstrap into the Stoa install process (OS-split obtention + registry-safe Windows PATH)
...
that the bash helper emits to a temp `.ps1` and invokes via `-File`. This one decision resolves F1
...
**Worktree HEAD (`stoa--elx/build`) — anchors re-confirmed this session:**
- `substrate/install.sh` = 2120 lines. `SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"`
  ...
  `scaffold_user_tier` defined L444, CALLED **L777** (this is where `bw init` fires). Flag DEFAULTS
  block L146-152. Header L3-135; `usage()` L272 renders the L3-135 block via `sed`.
...
## DC2.0 — F1 MECHANISM DECISION + EMPIRICAL git-bash ROUND-TRIP PROOF (the load-bearing rev2 core)

### The mechanism: PowerShell .NET registry API via an emitted `-File` script (NOT `reg.exe`)
...
**Invocation contract (the bash helper emits this, does not depend on `reg.exe` at all):** the helper
writes the script to `"$tmp/win_path.ps1"` (heredoc) and runs:
```
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$(cygpath -w "$tmp/win_path.ps1")" \
...
path without touching the real key), `-DryRun`, `-Ceiling`.

### EMPIRICAL PROOF — the actual commands I ran this session (git-bash), and their output
...
one. **ADA builds against this exact structure.**

---

## DC2 — Windows USER PATH mutation (the crux; DAEDALUS-owned)
...
### The registry-safe mutation — `win_path.ps1` (emitted by the helper; the mechanism proven in DC2.0)
...
`dir_win` via `cygpath -w`, emits `win_path.ps1`, and invokes it per the DC2.0 contract. The script's
logic (validated end-to-end in DC2.0):
...
The bash helper maps the exit code / RESULT line:
...
- **F2 resolved:** a failed/ambiguous read is `fail_read`/`fail_exception` (distinct non-zero exit +
  ...
  USER PATH" clobber is structurally impossible.
...
## F3 — composed-PATH length gate (web-verified this session; the 2047→4095 correction)
...
## DC3 — two-independent-checks (mandatory; the false-green killer) + F5 probe hardening
...
place a stub `bw.exe` in a throwaway dir, **`export PATH=<stub>:$PATH` in the git-bash session (so it
IS on git-bash's inherited env)** WHILE that dir is **absent from the registry User PATH** (never
written). Then:
...
P4 asserts the correct check exits 1 on this fixture; a build that implemented the naive form would
exit 0 and FAIL P4. This makes P4 discriminate the real implementation from the false-green
look-alike. (Confirmed this session that a child `powershell.exe` inherits a git-bash-exported PATH
entry — the asymmetry the fixture relies on is real.)
...
## DC6 — `install.sh` opt-in `--bootstrap-bw` (default OFF; absent = byte-unchanged) + F6/F7 fold-in

Four minimal edits (rev1 (1)/(2)/(4) unchanged; (3) MOVED per F6; invocation form changed per F7):
...
**(3) Pre-flight invocation — MOVED per F6.** rev1 placed this after the parse-loop `done` (L749),
i.e. BEFORE the target-required check at L753 — so `install.sh --bootstrap-bw` with a missing/invalid
`--target` would run a real download + PATH mutation *before* erroring on the missing target. **Fix:
place the guarded block immediately AFTER `[ -n "$TARGET" ] || err "--target is required..."` (L753)
and BEFORE `scaffold_user_tier` runs `bw init` (L777).** This makes a missing `--target` error out
first (no download/mutation), while bw is still made available before `bw init`. `SCRIPT_DIR` (L155)
is in scope at L754+.
```sh
...
**Why absent-flag output is byte-unchanged:** `BOOTSTRAP_BW=0` ⇒ the pre-flight `if` is false (emits
nothing), the case arm is never taken, NO plan-section line is added. The block's new placement
(after L753) is still inside a false `if` when the flag is absent — **byte-unchanged holds regardless
of placement** (ARGUS confirmed). VERA P7.
...
**Scope guard (unchanged).** Helper + TWO in-repo consumers (DC6, DC7) ONLY; the u--9s2 cookie-cutter
call-site lands in u--9s2 (`--yes`-ready here). arc-19 "do not install bw" superseded-in-spirit, NOT
edited. `bw upgrade`/`bw-upgrade.md` untouched (helper bootstraps the FIRST binary only).
...
never setx; never %PATH% expansion.`
...
- `M2 → defeats_via_probe: P2 (registry-safe append + provably-absent-vs-read-failure + token-idempotent, THROWAWAY key) + P3 (composed-length fail-loud, no write)`
- `M3 → defeats_via_probe: P4 (git-bash-green / registry-blind false-green demonstrably caught — F5 fixture)`
...
block so it runs even on a mid-script exception** (lesson from this session).
...
- **P2 — registry-safe append, THROWAWAY (M2 attack-blocked + legit-unaffected).** Drive `win_path.ps1`
  with `-KeyName Environment_stoa_test` (the EXACT real code path): (a) absent-dir seed → appended
  exactly once, `RESULT=appended`; (b) present-dir seed → `RESULT=present`, no write; (c) REG_EXPAND_SZ
  seed with `%USERPROFILE%`-token → kind + token PRESERVED (read-back RAW equals seed + our literal);
  (d) token-form-stored entry (`%USERPROFILE%\.local\bin`) with expanded `-Dir` → detected present, NO
  double-append (F8); (e) **P2b:** seed the key ABSENT / force a read failure → `RESULT=fail_read`
  exit 3, NO write (F2 — never overwrite); (f) grep the helper: NO `setx`, NO `%PATH%` expansion, NO
  `reg add`/`reg query`. `finally` teardown; real key untouched.
- **P3 — composed-length fail-loud (M2 + M4).** `-KeyName Environment_stoa_test -Ceiling <small>` (or a
  seed whose composed-expanded > 4095) → `RESULT=fail_length` exit 4, `fail_loud_path` prints the exact
  GUI manual step, NO write (throwaway value unchanged). Legit half: a normal append under 4095 writes.
- **P4 — DC3 false-green demonstrably CAUGHT (M3; F5 fixture).** Stub `bw.exe` in a throwaway dir,
  `export PATH=<stub>:$PATH` in git-bash (ON git-bash's inherited env) WHILE that dir is absent from
  the registry User PATH → assert: check (a) PASSES; a naive `powershell -c "bw --version"` would exit
  0 (false-green); the CORRECT check (b) (registry-REASSIGNED `$env:PATH`) EXITS 1; the helper does NOT
  skip / does NOT report green. Never touches real PATH.
...
- **P7 — install.sh absent-flag byte-unchanged (DC6 regression bar).**
  ...
  EMPTY; repeat `--target project --project-dir <tmp>`. **Plus F6 probe:** `install.sh --bootstrap-bw`
  with NO `--target` → errors on `--target is required` with NO download and NO PATH mutation (assert
  the pre-flight did not fire before the target check).
...
- **P10 — authorship + conformance (close-gate).** `Author=` Denson Smith zero-foreign; §28.9 seat
  trailer present; NOMOS CONFORMANT on the final commit.
...
1. **The whole DC2 crux now depends on `powershell.exe` + the .NET registry API being present and
   ...
   truly absent the Windows branch cannot function at all, and that fails loud early. **ARGUS: confirm
   the `-File` invocation (not `-Command`) is what ADA builds — a `-Command` form re-opens an
   argv-mangling surface.**
...
## Residual questions for ARGUS (re-audit targets)

- **R1:** Confirm the `-File` (not `-Command`) invocation contract is the built form, and that the
  emitted `win_path.ps1` uses `DoNotExpandEnvironmentNames` on BOTH the read and the idempotent-check
  expansion — a build that reads expanded would freeze tokens on write-back.
- **R2:** Confirm the F2 discrimination as-built: is there ANY code path where a non-zero PS exit or a
  swallowed exception leads the bash helper to proceed to a write with `cur=''`? The design says no
  (fail_read/fail_exception → fail_loud_path, no write); audit that ADA wired the exit-code mapping
  exactly, with no `|| true` swallowing the PS exit.
- **R3:** Is 4095 the right ceiling, or should the gate carry a safety margin (e.g. 4000) below the
  cliff? I chose the exact web-confirmed 4095; a conservative margin would trade a hair of headroom for
  a slightly earlier fail-loud. Your call to confirm or tighten.
- **R4:** F9 honest-stance language on M1 — confirm the tightened "SHA256's marginal value over TLS is
  corruption + single-object tamper, NOT MITM, NOT origin-compromise" is candid and not weasel-worded.
