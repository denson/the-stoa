# Arc 52 ARC A — Threat-defeat prevention layer (design-rev3)
...
**Supersedes:** `design-rev2.md` (committed 27c7d1f) — rev2 preserved for the audit trail. rev2 was PASSED by ARGUS-rev2 and independently PASSED by the floor-manager; this rev3 is a SURGICAL micro-revision closing the single residual r6. rev1 (committed 87b6e5b) is preserved one level further back.
**Audited-against:** rev2's ARGUS-rev2 PASS + the floor-manager LOCKED r6 disposition (option 1a tighten + 1b name-the-gap). This file is the revision the focused ARGUS-rev3 re-confirm audits.
**Worktree:** `.claude/worktrees/arc-52-build/`; source canon edited is `substrate/` (re-deploys via `install.sh`).

---

## Changes from rev2 (the surgical r6 fix — exactly two edits)

ARGUS-rev2 PASSED rev2 with ONE narrow residual, r6, whose disposition the floor-manager LOCKED. **r6:** the §35.1(b) "named threat" definition described a ratification point as a moment a risk set is "blessed **before or during build**" — but A1's only firing mechanism is the SINGLE pre-build pre-ADA beat. So the definition prose ("or during build") OVER-REACHED the pre-build firing mechanism: a present-but-false coverage claim, the exact false-confidence seam §35.5's own honest-claim rule forbids. The LOCKED disposition is option (1a) + (1b):

| Edit | Closes | What changed |
|---|---|---|
| **(1a) Tighten the §35.1(b) definition prose** | **r6** | In the "named threat" definition, "blessed **before or during build**" → "blessed **before build**", so the definition prose matches A1's single pre-ADA firing mechanism and the directive's before-build scope. (Grep-confirmed: this was the ONLY occurrence of "during build" / "or during build" anywhere in the §35 canon block — no other coverage claim of that phrase survives.) |
| **(1b) Name the during-build gap as an honest residual in §35.5** | **r6** | A ONE-LINE named residual added to §35.5 (alongside its existing honest-claim paragraph + carve-out): **during-build ratification** — a threat ratified AFTER the pre-ADA beat has fired — is OUT of Arc A's before-build scope, an **Arc-B (detection) candidate**. Honest-claim cuts both ways: REMOVE the overclaim (1a) AND NAME the gap (1b), so it is visible, not silently dropped — mirroring how §35.5 already names threat-enumeration completeness as a residual and §35.7 names adjacent uncovered modes. |

**REJECTED dispositions (not implemented):** (2) naming an A2 re-dispatch loop-back as a during-build firing path — imports Arc-B scope into Arc A's before-build mechanism; (3) accept rev2 as-is — leaves the present-but-false "during build" coverage claim in shipped canon.

**r6 does NOT regress r1/r4 (locus-independence — confirmed):** "before build" still covers EVERY ratification accumulated before the pre-ADA beat fires, including the mid-arc PRINCIPAL/floor-manager scope ratification that is r4's case — that ratification happens before-build, so it stays in-window and stays covered. "before build" ≠ "only at the design-critique pause"; A1's unconditional sweep over every PRIOR ratification is unchanged. The tighten removes a temporal overclaim (during/after the build run), NOT the locus-breadth r4 required.

**Nothing else changed; rev2-clean preserved.** Every other section, probe (P1–P14), insertion point, literal canon block, and the rev2 "Changes from rev1" table below is carried forward BYTE-FOR-BYTE from rev2 (27c7d1f). The two edits above are the only substantive changes; the only additions are P15 (the r6 probe in §4) and this section. The self-classification (`not threat-ratified (process change, no runtime attack path)`) is unchanged.

---

## Changes from rev1 (each mapped to the ARGUS risk it closes) — carried forward from rev2 for the audit trail
...
**r6 fix (the overclaim is gone AND the gap is named — rev3):**

- **P15 (the "before or during build" overclaim is PURGED + during-build named as a residual — rev3 r6 fix).** `grep -n "during build" substrate/operating-disciplines.md` MUST return NO "**before or during build**" coverage claim in §35.1 — the §35.1(b) "named threat" definition now reads "blessed **before build**" (the temporal overclaim is gone). **AND** §35.5 MUST contain the during-build NAMED RESIDUAL — `grep -n "during-build\|Arc-B (detection) candidate\|before-build scope" substrate/operating-disciplines.md` matches the §35.5 residual line naming during-build ratification (a threat ratified AFTER the pre-ADA beat) as OUT of before-build scope / an Arc-B candidate. **Probe of the probe (r6):** does §35.1's definition prose claim coverage the A1 firing mechanism does not deliver? It MUST NOT — A1 fires once before build (§35.2), so the definition must say "before build", and the during-build gap must be NAMED not silently dropped. **Non-regression (r1/r4):** the tighten MUST NOT narrow locus-breadth — §35.1(b) still reads "**ANY ratification point**" with the mid-arc-scope-ratification enumeration intact (re-run P1: "including a mid-arc scope ratification, outside the design-critique pause" still present). "before build" removed a temporal overclaim, not the locus breadth r4 required.
...
- **W6 (rev3 r6 — scrutinize that the tighten did not narrow r4's locus-breadth).** The r6 fix changes §35.1(b) "before or during build" → "before build" (1a) and names during-build ratification as a §35.5 residual (1b). The brittle spot ARGUS-rev3 should re-check: "before build" is a TEMPORAL bound (when A1 fires — once, pre-ADA), NOT a locus bound (WHERE a ratification happened) — the two are orthogonal, and r4's mid-arc PRINCIPAL scope ratification happens before-build so it stays covered (re-run P1 + P15's non-regression clause). *Why this shape anyway:* the floor-manager LOCKED 1a+1b precisely because the overclaim was temporal (the definition said coverage extends "during build" when A1 only fires before it); removing the temporal overclaim and naming the during-build gap as an Arc-B residual is the honest-claim discipline cutting both ways. The only residual: a reader could conflate "before build" with "only at the design-critique pause" — §35.1(b)'s retained "**ANY ratification point**" + the mid-arc enumeration is the explicit guard against that misread, and W1 already owns the broader inclusive-not-vague test.
...
- **During-build ratification** (a threat ratified AFTER the pre-ADA A1 beat has fired) — named as a §35.5 residual / Arc-B (detection) candidate by the rev3 r6 fix; OUT of Arc A's before-build scope, not a coverage gap this prevention layer claims to close.

---

*Authored for the PRINCIPAL (Denson Smith). No author-like field is touched by these edits. This design is itself classified `not threat-ratified (process change, no runtime attack path)` per §35.5 — the focused ARGUS-rev3 re-confirm is invited to confirm that classification (carried unchanged from rev2's ARGUS-confirmed state).*
