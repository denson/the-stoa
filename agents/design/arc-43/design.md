...
**Target location:** `substrate/operating-disciplines.md`, append a new section 32 after the existing 31 (substrate-component design principles — Arc 38 ship at line 1971). Heading shape: `## 32. Test-environment timing discipline — jsdom + animation libraries` (top-level `##` heading + dot-after-number, per existing op-disc convention — see `## 31. Substrate-component design principles for agent-installable distribution` at line 1971).
...
```bash
# CAPTAIN_DAEDALUS.md §6.9 cross-ref prose cites §6.9.3' and §6.9.3''
grep -nE "§6\.9\.3'" substrate/CAPTAIN_DAEDALUS.md | head -10

# §6.2 cross-ref prose cites §6.2.1' and §6.10
grep -nE "§6\.2\.1'|§6\.10" substrate/CAPTAIN_DAEDALUS.md | head -10

# §6.11 closing cross-refs cite CAPTAIN_ADA.md §5.9
grep -nE "CAPTAIN_ADA\.md.*§5\.9|§5\.9.*CAPTAIN_ADA" substrate/CAPTAIN_DAEDALUS.md

# op-disc §32 cross-refs cite CAPTAIN_ADA.md §5.9 and CAPTAIN_VERA.md
grep -nE "§5\.9|CAPTAIN_VERA" substrate/operating-disciplines.md | tail -10
```
...
Acceptance criteria (the falsification thresholds):
- **check-1 FAIL count drops from 144 to ≤30.** Most of the 144 FAILs are spec-self-heading false-positives (~120 of them are heading-line `### §N.X` patterns); the heading-self-skip + spec-self-resolve fix should eliminate substantially all. ≤30 is the conservative threshold; lower is better.
- **check-2 STRANGE count drops from 36 to ≤10.** Most STRANGEs are `- CN: stoa--XXX <description>` enumeration lines (~28 of them) under DONE-context sections; the section-context fallback should classify them as closed. Plus `Follow-up filed:` (~4) → matches `filed` keyword → open. ≤10 is the conservative threshold.

If the deltas are smaller than expected, that is data, not failure — it means the parser limitations have a residue beyond the Arc 43 fix scope (per stoa--bbi refined-principle thesis). The acceptance is the threshold, not all-green.

**Round-trip-adjacent-prose audit (§6.9.3'):** the "≤30 / ≤10 thresholds" prose claims are contract thresholds VERA verifies. Round-trip: the spec_refs.py summary emits `{"summary": true, "total": N, "pass": N, "fail": N, "strange": N}`; the verification is to parse the summary JSON and assert `fail ≤ 30`. The bw_tickets.py check-2 summary emits the same shape; assert `strange ≤ 10`. Both summaries are JSONL lines; VERA can read with `python -c "import json,sys; print(json.loads(sys.stdin.read()))"` or jq.
...
4. **Weak point — Probe 4 acceptance thresholds (≤30 check-1 FAIL; ≤10 check-2 STRANGE) are conservative estimates, not derived from the underlying SPECIFICATION.md by ADA-time.** I estimated based on a mental count of the FAIL/STRANGE pattern distribution in mech-check-results.md (~120 of 144 check-1 FAILs are heading-line patterns; ~28 of 36 check-2 STRANGEs are §13.X enumeration). If the actual counts are different (e.g., the parser fix catches fewer than I estimated), VERA's verdict may need to interpret the residue rather than declare PASS/FAIL against the threshold. **Why this shape anyway:** thresholds are intentionally conservative ("≤30" not "0") to allow for parser-residue per stoa--bbi; the acceptance is the threshold, not all-green. If even the conservative thresholds aren't met, the residue is data (and a follow-up).
...
- The 60× cost-multiplier math (weak point 3): is "60×" too specific for canonical wording? Acceptable softening to "substantial multiplier" if you flag this.
- The A4=ε Dev2 op-disc §32 pick (weak point 1): does the universality-across-CAPTAINs argument hold, or should this land at CAPTAIN_ADA.md §5.10? Either acceptable; pivot at rev1 if you surface a structural reason for the latter.
- The cross-line spec-self-resolve in spec_refs.py (weak point 2): the lean named per-line skip; my implementation extends to non-heading prose. If you flag this as scope-creep beyond A8=η, I narrow back to per-line-only and live with the ~30 remaining FAILs.
- Recursive self-application surveillance: any §6.9.3' / §6.9.3'' / §6.2.1' violation you catch in THIS design's own probes counts as a POSITIVE empirical anchor for the canon being shipped (per Pass 10 precedent). I've tried to apply the disciplines honestly; expect you to catch what I missed.
