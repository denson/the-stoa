<!--
ARCHIVED — v1 paste-instruction template.

This file is preserved for historical reference only. It uses v1 terminology
("Colonel" as the title for the human served by the system) in the rationale
prose, which v2 corrects: PRINCIPAL is the human's descriptive role; COLONEL
is reserved as a future high-autonomy agent rank. The substitution slots
({{PROJECT_NAME}}, {{SESSION_INTENT}}, {{BW_PREFIX}}, {{ROLE_FILE_PATH}},
{{PENDING_DIRECTIVES}}, {{ON_DISK_PATH}}) and the string-substitution
mechanism are preserved in v2 — the voice changes; the mechanism does not.

Canonical successor: ../../templates/paste-instruction-template.md (v2 —
